...
function timeDelta(total: number, expected: number): { label: string; color: string; bg: string } {
  ...
  if (diffMin === 0) return { label: "準時完成", color: "text-slate-500", bg: "bg-slate-50" }
  if (diff > 0) return { label: `超時 ${diffMin} 分`, color: "text-red-500", bg: "bg-red-50" }
  return { label: `快 ${diffMin} 分`, color: "text-amber-600", bg: "bg-amber-50" }
}
